...
class JointGaussianDataset:
    """
    generates data from a uniform range, takes in two transforms to go from
    A -> B
    B -> C
    for multi-modal contrastive training
    """
    def __init__(self, mu, sigma, transform1, transform2, pair, dims=(2, 2, 2), num_points=1000, noise=(0.01, 0.01)):
        assert transform1.shape == (dims[1], dims[0]), "transform1 must be of shape (B_dim, A_dim)"
        assert transform2.shape == (dims[2], dims[0]), "transform2 must be of shape (C_dim, B_dim)"

        A_dim, B_dim, C_dim = dims
        eps1 = np.random.randn(num_points, B_dim) * noise[0]  # Adjusted shape for noise addition
        eps2 = np.random.randn(num_points, C_dim) * noise[1]  # Adjusted for transform2's output dimension

        self.A = np.random.multivariate_normal(mu, sigma, num_points)
        self.B = np.dot(self.A, transform1.T) + eps1
        self.C = np.dot(self.A, transform2.T) + eps2
...
def computeAccuracy(model, dataset, dims=(2, 2, 2), unconscious=True, points=None, dist_loss=True):
    """
...
def train_unconscious(model, loss_function, 
    ...
    for epoch in range(num_epochs):
        acc = computeAccuracy(model, val_AB, dims=dims, unconscious=True, points=torch.tensor(AB_dataset.points), dist_loss=dist_loss)
        val_accs.append(acc)  
...
def runExperiment(input_dims, 
    ...
    # define datasets
    AB_dataset = JointGaussianDataset(mu, Sigma, transform1=transform1, transform2=transform2, pair=[0, 1], dims=DIMS, num_points=TRAIN_SIZE, noise=NOISE)
    BC_dataset = JointGaussianDataset(mu, Sigma, transform1=transform1, transform2=transform2, pair=[1, 2], dims=DIMS, num_points=TRAIN_SIZE, noise=NOISE)
    AC_dataset = JointGaussianDataset(mu, Sigma, transform1=transform1, transform2=transform2, pair=[0, 2], dims=DIMS, num_points=TRAIN_SIZE, noise=NOISE)

    # validation data
    val_AB = JointGaussianDataset(mu, Sigma, transform1=transform1, transform2=transform2, pair=[0, 1], dims=DIMS, num_points=VAL_SIZE, noise=NOISE)
    val_BC = JointGaussianDataset(mu, Sigma, transform1=transform1, transform2=transform2, pair=[1, 2], dims=DIMS, num_points=VAL_SIZE, noise=NOISE)
    val_AC = JointGaussianDataset(mu, Sigma, transform1=transform1, transform2=transform2, pair=[0, 2], dims=DIMS, num_points=VAL_SIZE, noise=NOISE)
    ##########################
...
def main():
    ...
    experiments_base_dir = 'experiment1'
    ...
    # Hyperparameters
    l2_penalties = [0.0, 0.001, 0.01]# [0.0, 0.001, 0.01, 0.1, 1.]
    feature_dims = [2, 4] 
    temperatures =  [0.01, 0.1] #[0.001, 0.01, 0.1, 1.]
    NUM_TRIALS = 5
    ...
    for trial in range(NUM_TRIALS):  # Fix the loop variable here
        print(f'trial {trial}')
        mu = np.random.uniform(-1, 1, (input_dims[0]))
        A = np.random.uniform(-1, 1, (input_dims[0], input_dims[0]))
        Sigma = A.T @ A

        transform1 = np.random.uniform(-1, 1, (input_dims[1], input_dims[0]))
        transform2 = np.random.uniform(-1, 1, (input_dims[2], input_dims[0]))
        ...
        for l2_penalty in l2_penalties:
            for feature_dim in feature_dims:
                for temperature in temperatures:
                    ...
                    exp_dir = f"trial_{trial}_l2_{l2_penalty}_fd_{feature_dim}_temp_{temperature}"
    ...
    df_csv_path = os.path.join(experiments_base_dir, 'experiment_details.csv')
